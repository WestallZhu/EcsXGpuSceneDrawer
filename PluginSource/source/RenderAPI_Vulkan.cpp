...
public:
    ...
    virtual void UpdateTexture2DSub(void* textureHandle, int xoffset, int yoffset, int width, int height, int pixelsByte, void* dataPtr);
...
void RenderAPI_Vulkan::UpdateTexture2DSub(void* textureHandle, int xoffset, int yoffset, int width, int height, int pixelsByte, void* dataPtr)
{
    if (!textureHandle || !dataPtr || width <= 0 || height <= 0 || pixelsByte <= 0)
        return;

    UnityVulkanRecordingState stagingState;
    if (!m_UnityVulkan->CommandRecordingState(&stagingState, kUnityVulkanGraphicsQueueAccess_DontCare))
        return;

    SafeDestroy(stagingState.currentFrameNumber, m_TextureStagingBuffer);
    m_TextureStagingBuffer = VulkanBuffer();

    const size_t rowPitch = static_cast<size_t>(width) * static_cast<size_t>(pixelsByte);
    const size_t stagingSize = rowPitch * static_cast<size_t>(height);
    if (!CreateVulkanBuffer(stagingSize, &m_TextureStagingBuffer, VK_BUFFER_USAGE_TRANSFER_SRC_BIT))
        return;

    memcpy(m_TextureStagingBuffer.mapped, dataPtr, stagingSize);

    if (!(m_TextureStagingBuffer.deviceMemoryFlags & VK_MEMORY_PROPERTY_HOST_COHERENT_BIT))
    {
        VkMappedMemoryRange range;
        range.sType = VK_STRUCTURE_TYPE_MAPPED_MEMORY_RANGE;
        range.pNext = NULL;
        range.memory = m_TextureStagingBuffer.deviceMemory;
        range.offset = 0;
        range.size = m_TextureStagingBuffer.deviceMemorySize;
        vkFlushMappedMemoryRanges(m_Instance.device, 1, &range);
    }
