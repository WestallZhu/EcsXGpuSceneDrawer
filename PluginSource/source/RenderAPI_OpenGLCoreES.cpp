...
#	define GL_LOG(fmt, ...) emscripten_log(EM_LOG_CONSOLE, fmt, ##__VA_ARGS__)
#else
#	define GL_LOG(fmt, ...) printf(fmt, ##__VA_ARGS__)
#endif
...
void RenderAPI_OpenGLCoreES::UpdateTexture2DSub(void* textureHandle, int xoffset, int yoffset, int width, int height, int pixelsByte, void* dataPtr)
{
	//GL_LOG("[WebGL] UpdateTexture2DSub tex=%p offset=(%d,%d) size=(%d,%d) pixelBytes=%d dataPtr=%p\n",
	//	textureHandle, xoffset, yoffset, width, height, pixelsByte, dataPtr);
	GLuint gltex = (GLuint)(size_t)(textureHandle);

	 //Update texture data, and free the memory buffer
	glBindTexture(GL_TEXTURE_2D, gltex);
	//GLenum err0 = glGetError();
	//if (err0 != GL_NO_ERROR) { GL_LOG("[WebGL] glBindTexture err=0x%X\n", err0); return; }

	int type = pixelsByte == 16 ? GL_FLOAT : GL_UNSIGNED_BYTE;
	glTexSubImage2D(GL_TEXTURE_2D, 0, xoffset, yoffset, width, height, GL_RGBA, type, dataPtr);

	//err0 = glGetError();
	//if (err0 != GL_NO_ERROR) { GL_LOG("[WebGL] glTexSubImage2D err=0x%X\n", err0); return; }
	//GL_LOG("[WebGL] UpdateTexture2DSub complete for tex=0x%X\n", gltex);
}
