...
void RenderAPI_Metal::UpdateTexture2DSub(void* textureHandle, int xoffset, int yoffset, int width, int height, int pixelsByte, void* dataPtr)
{
	if (!textureHandle || !dataPtr || width <= 0 || height <= 0 || pixelsByte <= 0)
		return;

	id<MTLTexture> tex = (__bridge id<MTLTexture>)textureHandle;
	const NSUInteger bytesPerRow = (NSUInteger)(width * pixelsByte);
	MTLRegion region = MTLRegionMake2D(xoffset, yoffset, width, height);
	[tex replaceRegion:region mipmapLevel:0 withBytes:dataPtr bytesPerRow:bytesPerRow];
}
