...
namespace Unity.Entities
{
    ...
    public unsafe partial struct EntityManager : IEquatable<EntityManager>
    {
        ...
        public void AddChunkComponentData<T>(NativeArray<ArchetypeChunk> chunks, T componentData) where T : unmanaged, IComponentData
        {
            var access = GetCheckedEntityDataAccess();
            var ecs = access->EntityComponentStore;
        ...
        public void RemoveChunkComponentData<T>(NativeArray<ArchetypeChunk> chunks) where T : unmanaged, IComponentData
        {
            var access = GetCheckedEntityDataAccess();
            var ecs = access->EntityComponentStore;
            ...
            if (chunks.Length > 0)
            {
                ...
                var componentType = ComponentType.ReadWrite<T>();
                var componentTypeIndex = componentType.TypeIndex;
                var componentTypeIndexForRemove = TypeManager.MakeChunkComponentTypeIndex(componentTypeIndex);


                var changes = access->BeginStructuralChanges();
                StructuralChange.RemoveComponentChunks(ecs, chunkPtr, chunks.Length, componentTypeIndexForRemove);
                access->EndStructuralChanges(ref changes);
            }
        }
